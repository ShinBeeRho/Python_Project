...
def execute_crawler():
    ...
    for code in CODES:
        res = requests.get(BASE_URL + str(CODES[0]))    # 전체 페이지를 가져오는 코드
        page_soup = BeautifulSoup(res.text, 'lxml')

        total_page_num = page_soup.select_one('td.pgRR > a')
        total_page_num = int(total_page_num.get('href').split('=')[-1])

        ipt_html = page_soup.select_one('div.subcnt_sise_item_top')     # 조회할 수 있는 항목 정보들 추출
